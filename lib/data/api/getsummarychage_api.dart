...
class GetSummaryDischargeApi {
  //http://localhost:3000/api/information/summaryCharge/1
 // final String token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MjEsInVzZXJJRCI6MzAsInR5cGUiOiJvYmplY3QiLCJzcGVjaWFsaXN0IjoiZ2VuZXJhbCIsImlhdCI6MTcyMzMxNzU4NH0.ZFJr4q0D_mxsWxgcyDRNvhybVhomP35SQTo-xkb9h48';
  ...
  static Future<List<SummaryDischarge>> fetchSummary() async {
    ...
    if (response.statusCode == 200) {
      ...
      final data = jsonDecode(response.body);
      final summaryDischarges = (data['summaryDischarge'] as List)
          .map((json) => SummaryDischarge.fromJson(json))
          .toList();

      return summaryDischarges;
    } else {
      throw Exception('Failed to load SummaryDischarges');
    }
  }
}
